...
def set_param():
    ...
    # 是否根据EPS选股
    g.pick_by_eps = True
    # 配置选股最小EPS值，EPS是每股盈余（Earnings Per Share）的缩写，指普通股每股税后利润，也称为“每股收益”。
    if g.pick_by_eps:
        g.min_eps = 0
...
def handle_data(context, data):
    # 止损策略
    # 根据大盘历史价格止损：大盘指数前130日内最高价超过最低价2倍，则清仓止损
    if g.is_market_stop_loss_by_price:
    ...
    # 根据28指数值实时进行止损：当日28指数连续为跌的分钟计数达到指定值则止损
    # 根据28指数值实时进行止损：当前28指数同时且连续120分钟小于前20天收盘价则止损 -> xudaosong remark
    if g.is_market_stop_loss_by_28_index:
...
def market_stop_loss_by_price(context, index):
    # 大盘指数前160日内最高价超过最低价2.2倍，并且前1天的收盘价小于前4天的收盘价，并且前1天的收盘价大于前100天的收盘价则清仓止损
    ...
    if not g.is_day_stop_loss_by_price:
        ...
        # 大盘指数前160日内最高价超过最低价2.2倍 并且
        # 当前时间前1天的收盘价小于前4天的收盘价 并且
        # 当前时间前1天的收盘价大于前100天的收盘价 并且
...
def pick_stocks(context, data):
    q = query(valuation.code) # 连接数据库
    if g.pick_by_pe: # 按股票的PE过滤
        q = q.filter(
            valuation.pe_ratio > g.min_pe,
            valuation.pe_ratio < g.max_pe
            )

    if g.pick_by_eps: # 过滤亏损的股票
        q = q.filter(
            indicator.eps > g.min_eps,
            #valuation.turnover_ratio > 3
            )

    q = q.order_by( # 按总市值升序排序，并取出前N个股票
                valuation.market_cap.asc()
            ).limit(
                g.pick_stock_count
            )

    df = get_fundamentals(q) # 获取查询结果股票的财务数据
    stock_list = list(df['code']) # 获取查询结果所有股票的代码

    if g.filter_gem: # 从选出股票过滤掉创业板股票
        stock_list = filter_gem_stock(context, stock_list)

    if g.filter_blacklist:# 从选出股票过滤掉黑名单股票
    ...
    if g.is_rank_stock: # 是否对股票评分
        if len(stock_list) > g.rank_stock_count: # 如果股票池里的股票数 大于 参与评分的股票数，则取出前N个股票
            stock_list = stock_list[:g.rank_stock_count]

        #log.debug("评分前备选股票: %s" %(stock_list))
        if len(stock_list) > 0:
            stock_list = rank_stocks(data, stock_list) # 获取评分后的股票，按分数从高到低排序
